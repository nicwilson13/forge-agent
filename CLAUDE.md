...
- **Pure utility modules** (`retry.py`, `context_budget.py`, `build_logger.py`, `memory.py`, `advanced_options.py`, `display.py`, `router.py`, `mcp_config.py`, `dependency_graph.py`, `workflow_generator.py`, `figma_integration.py`, `vercel_integration.py`, `linear_integration.py`, `sentry_integration.py`) have zero imports from other forge modules (except `dependency_graph.py` which imports only `forge.state`) — they are imported by others, never the reverse.
...
- **`.forge/` directory** is the single location for all runtime state: `state.json`, `cost_log.jsonl`, `build.log`, `memory/`, `mcp.json`, `github.json`, `figma.json`, `vercel.json`, `linear.json`, `sentry.json`.
