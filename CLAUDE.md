...
**orchestrator.py** — Calls Anthropic API (Claude Opus) to generate phases, generate tasks, write ARCHITECTURE.md, evaluate QA, and review phases. All API calls go through `_chat()` which has built-in retry with exponential backoff and optional `mcp_servers` parameter for MCP integration. `_json_chat()` wraps `_chat()` with JSON parsing and retries once on `JSONDecodeError`. `build_task_prompt()` uses `ContextBudget` for intelligent truncation instead of hardcoded char limits; injects skill packs based on signal detection (FRONTEND_SIGNALS, DATABASE_SIGNALS, AUTH_SIGNALS, PAYMENTS_SIGNALS). All public functions (`generate_phases`, `generate_tasks`, `write_architecture`, `evaluate_qa`, `evaluate_phase`) accept an optional `mcp_config` parameter. `generate_tasks()` also accepts `github_issues_context`, `figma_context`, and `linear_context` to enrich task planning.

**builder.py** — Calls Claude Code SDK (`claude_code_sdk.query()`) to execute each task. Streams output in real time. Returns `(success, stdout, stderr, duration)` with error prefixes: AUTH_ERROR, RATE_LIMIT, CONNECTION_ERROR, TIMEOUT, PROCESS_ERROR, SDK_ERROR. `find_claude_cli()` locates the CLI with platform-specific fallbacks (Windows `%APPDATA%\npm\claude.cmd`, Unix `~/.local/bin/claude`, etc.) for virtualenv compatibility. `_resolve_cli_for_transport()` resolves Windows `.cmd` wrappers to actual `.exe` for `SubprocessCLITransport` compatibility.

**commands/run.py** — The main loop. `run_forge()` handles initial setup, then iterates phases/tasks. `_print_startup_diagnostics()` displays pre-flight checks (masked API key, Claude CLI path, SDK availability) for immediate proof-of-life. `_TeeWriter` wraps stdout to duplicate all output to `.forge/run_output.log` for post-mortem debugging; only installed when `sys.stdout.isatty()` to avoid double-write when launched as a subprocess. `_execute_task()` calls builder, runs tests, evaluates QA, commits on success, retries or parks on failure. `_complete_phase()` runs E2E tests, security scan, phase QA evaluation, and GitHub integration (milestone, PR, build summary) before advancing. Signal handlers save checkpoint on Ctrl+C. `FatalAPIError` exits with code 1 (auth broken), `RetryExhaustedError` exits with code 0 (resumable pause). Loads `MCPConfig` and `GitHubConfig` at startup and threads them through all orchestrator calls.

**state.py** — Dataclasses: `ForgeState` → `Phase` → `Task`. Task statuses: PENDING → IN_PROGRESS → DONE/FAILED/PARKED/INTERRUPTED/COMMIT_PENDING/WAITING. State persists to `.forge/state.json`. Task IDs are 8-char UUID substrings via `Task.new()`. `Task.depends_on: list[str]` declares dependencies on other task IDs within the same phase. `Phase.github_milestone` and `Phase.github_pr` track GitHub integration state. `load_state()` uses `_filter_fields()` to strip unknown keys for forward compatibility and handles corrupted JSON gracefully. `TaskStatus(str, Enum)` inherits from `str`, so enum values work correctly with string comparisons from JSON deserialization.
...
**dashboard.py** — Local web dashboard at `localhost:3333` for live build monitoring. Uses `_ThreadedHTTPServer` (ThreadingMixIn) so SSE doesn't block other routes. SSE push via `push_event()` and `update_dashboard_state()`. Routes: `/` (build overview), `/tasks` (NEEDS_HUMAN task management), `/history` (past build records), `/integrations` (config management), `/linear` (Kanban board), `/setup` (project wizard), plus JSON data endpoints and `/events` (SSE stream). `start_dashboard(run_mode=True)` when called by `forge run` prevents redirect to `/setup` during initial phase generation (before `state.json` exists). `stop_dashboard()` for graceful shutdown. All page views use `nav_shell.page_shell()` for consistent layout.

**setup_wizard.py** — Multi-step browser-based project setup wizard as alternative to terminal interview. `handle_setup_get()` serves the wizard UI, `handle_setup_submit()` writes VISION.md/REQUIREMENTS.md from form data, `handle_ai_assist()` calls Claude to draft content, `start_forge_run_subprocess()` spawns `forge run` as a detached process with stdout redirected to `.forge/run_output.log`. Imports only `nav_shell`.

**tasks_view.py** — NEEDS_HUMAN task management UI. `get_parked_tasks()` finds parked/failed tasks, `resolve_task()` updates task status (retry or skip), `handle_tasks_get/data/resolve` serve HTML and JSON endpoints. Imports only `nav_shell`.
...
**sentry_integration.py** — Queries Sentry for unresolved errors after phase deployments and creates fix tasks automatically. `SentryConfig` dataclass with per-project settings in `.forge/sentry.json` (enabled, org_slug, project_slug, auto_configure, create_fix_tasks, error_threshold). Uses Sentry REST API (`https://sentry.io/api/0/`). `_sentry_request()` uses urllib with Bearer token auth, 15s timeout, never raises. `get_unresolved_issues()` fetches issues above `error_threshold` (default 5). `get_recent_events()` fetches stack trace context. `format_issue_as_fix_task()` converts Sentry issue to `(title, description)` pair. `generate_sentry_setup_instructions()` returns @sentry/nextjs auto-config task. `run_sentry_check()` is the full pipeline called after phase deploy. Fix tasks injected into next phase via `_inject_task_into_next_phase()` in run.py. Sentry token stored in `~/.forge/profile.yaml` under `sentry_token`. Integrated into `forge new` and `forge doctor`. Zero forge imports (stdlib only). Never raises.
...
**commands/new.py** — Guided AI interview to generate VISION.md, REQUIREMENTS.md, CLAUDE.md. Uses Anthropic API for question generation and doc synthesis. Integrates `advanced_options.py` for optional detailed configuration. Post-interview optional steps: MCP server setup (`_offer_mcp_setup()`), GitHub integration setup (`_offer_github_setup()`), GitHub Actions CI workflow generation (`_offer_workflow_generation()`), Vercel setup (`_offer_vercel_setup()`), Figma setup (`_offer_figma_setup()`), Linear setup (`_offer_linear_setup()`), Sentry setup (`_offer_sentry_setup()`), and Ollama setup (`_offer_ollama_setup()`).
...
- **Atomic file writes** for checkpoint and build_logger: write to `.tmp`, then `tmp.replace(target)`. Cost_tracker and memory use append mode (`open("a")`) for safe concurrent writes in parallel execution.
- **display.py** symbols (`SYM_OK`, `SYM_FAIL`, `SYM_WARN`) have ASCII fallbacks when stdout doesn't support Unicode.
- **`.forge/` directory** is the single location for all runtime state: `state.json`, `cost_log.jsonl`, `build.log`, `run_output.log`, `build_history.json`, `memory/`, `mcp.json`, `github.json`, `figma.json`, `vercel.json`, `linear.json`, `sentry.json`, `ollama.json`.
- **Sensitive tokens** (GitHub, Figma, Vercel, Linear, Sentry) are stored in `~/.forge/profile.yaml` — never in the project directory.
- **Phase completion flow** in `_complete_phase()`: E2E tests → security scan → `evaluate_phase()` → GitHub integration (milestone, PR, build summary) → Vercel deployment check → Sentry error check → tag/advance. Security and E2E results are passed to `evaluate_phase()` for the full picture. Sentry fix tasks are injected into the next phase via `_inject_task_into_next_phase()`.
- **Never-raise convention** for quality gate modules (`diff_review`, `security_scan`, `e2e_generator`), MCP config loading, GitHub integration, Figma integration, Vercel integration, Linear integration, Sentry integration, and workflow generation: all public functions catch exceptions and return safe defaults so the build loop is never crashed by a quality check, missing config, or API failure.
