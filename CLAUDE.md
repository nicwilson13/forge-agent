...
**orchestrator.py** — Calls Anthropic API (Claude Opus) to generate phases, generate tasks, write ARCHITECTURE.md, evaluate QA, and review phases. All API calls go through `_chat()` which has built-in retry with exponential backoff and optional `mcp_servers` parameter for MCP integration. `_json_chat()` wraps `_chat()` with JSON parsing and retries once on `JSONDecodeError`. `build_task_prompt()` uses `ContextBudget` for intelligent truncation instead of hardcoded char limits. All public functions (`generate_phases`, `generate_tasks`, `write_architecture`, `evaluate_qa`, `evaluate_phase`) accept an optional `mcp_config` parameter. `generate_tasks()` also accepts `github_issues_context` to enrich task planning with open GitHub issues.

**builder.py** — Calls Claude Code SDK (`claude_code_sdk.query()`) to execute each task. Streams output in real time. Returns `(success, stdout, stderr, duration)` with error prefixes: AUTH_ERROR, RATE_LIMIT, CONNECTION_ERROR, TIMEOUT, PROCESS_ERROR, SDK_ERROR. `find_claude_cli()` locates the CLI with platform-specific fallbacks (Windows `%APPDATA%\npm\claude.cmd`, Unix `~/.local/bin/claude`, etc.) for virtualenv compatibility.
...
- **`.forge/` directory** is the single location for all runtime state: `state.json`, `cost_log.jsonl`, `build.log`, `memory/`, `mcp.json`, `github.json`, `figma.json`, `vercel.json`, `linear.json`, `sentry.json`, `ollama.json`.
